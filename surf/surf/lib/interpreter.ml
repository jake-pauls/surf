...
(** [print_error_position lexbuf] retrieves the line and position of a lexer
    error for printing *)
...
let parse_token (s : string) : expr option =
  ...
  | Parser.Error ->
      print_endline
        (Fmt.str "[Syntax Error ~> %s]" (print_error_position lexbuf))
      ; None
...
(** [interp_ret] interprets [s] by lexing, parsing, evaluating it, and returning it as a string *)
